...
package object util {
  ...
  case class Bin(bin:ArrayBuffer[Kmer], var size:Int){
    ...
    class KmerWithSequence(seq: Short,length: Int,from:Kmer,startPos:Int,endPos:Int,orientation:Int) extends Kmer(length,from,startPos,endPos,orientation){
      def getSequence: Short = seq
